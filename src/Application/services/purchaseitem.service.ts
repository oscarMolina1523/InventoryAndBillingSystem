...
export class PurchaseItemService implements IPurchaseItemService {
  ...
  constructor(@inject("IPurchaseItemRepository") repository: IPurchaseItemRepository) {
    this._purchaseitemRepository = repository;
  }
  
  async findAll(page: number = 1, pageSize: number = 100): Promise<PurchaseItem[]> {
  ...
  async update(id: string, data: PurchaseItemDto): Promise<PurchaseItem | null> {
