...
export class CashMovementService implements ICashMovementService {
  ...
  constructor(@inject("ICashMovementRepository") repository: ICashMovementRepository) {
    this._cashmovementRepository = repository;
  }
  
  async findAll(page: number = 1, pageSize: number = 100): Promise<CashMovement[]> {
  ...
  async update(id: string, data: CashMovementDto): Promise<CashMovement | null> {
